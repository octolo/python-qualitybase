"""Dynamic import and class inheritance decorators."""
...
def _import_string(dotted_path, package=None):
    if dotted_path.startswith('.'):
        ...
        try:
            module = importlib.import_module(dotted_path, package=package)  # nosec B307 # nosemgrep
            return module
    ...
    for path in paths_to_try:
        try:
            ...
            module = importlib.import_module(module_path)  # nosec B307 # nosemgrep
...
def inherit_class_list(*args):
    """Decorator to inherit from classes specified by dotted path strings."""
    def decorator(cls):

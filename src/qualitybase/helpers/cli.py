...
def _discover_from_path(  # pylint: disable=too-many-branches,too-many-nested-blocks
    commands_path: Path, package_name: str
) -> dict[str, CommandInfo]:  # noqa: C901
    ...
    try:
        ...
        Command = base_module.Command  # pylint: disable=invalid-name
...
def discover_commands(  # noqa: C901  # pylint: disable=too-many-locals
    cli_file_path: Path,
    ...
    if config_file.exists():
        ...
        if "packages" in config:
            for pkg in config["packages"]:
                pkg_commands = _discover_from_package(pkg)
                result.update(
                    {k: v for k, v in pkg_commands.items() if v.get("inherit", True)}
                )
        ...
        for _ in range(20):
            ...
            if frame:
                ...
                if frame_file:
                    ...
                    if (
                        "cli.py" in frame_file
                        and frame_path != qualitybase_cli_path
                    ):
...
def cli_main(  # pylint: disable=too-many-locals
